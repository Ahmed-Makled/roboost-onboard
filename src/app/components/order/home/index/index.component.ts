...
import { OrderHomeService } from '../home.service';
...
export class IndexComponent extends CrudIndexBaseUtils implements OnInit {
  ...
  constructor(
    private _pageService: OrderHomeService,
  ...
  ngOnInit() {
    this.isArchive = this._router.url.split('/')[2] ? this._router.url.split('/')[2].split('?')[0] == "archived" : false
    this.pageRoute = '/task' + (this.isArchive ? '/archived' : '/recent-tasks')
    this.initializePage();
  }
  initializePage() {
  ...
  getColors() {
    this.items.forEach(element => { element.StatusColor = this._statusUtilsService.getTaskRateColors(element.Rate) });
  }
  createRandomOrder() {
    this._pageService.creatRandomOrder().subscribe(res => {
  ...
  canCreateRandOrder() {
    this._pageService.CanCreateRandomOrder().subscribe(res => {
      if (res.Success) this.canCreateRandomOrder = res.Data
    })
  }
